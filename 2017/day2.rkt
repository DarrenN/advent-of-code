...
;; Convert the big string of rows into a list of lists of digits
;; '((5 1 9 5) (7 5 3) (2 4 6 8))
(define (create-sorted-rows sheet)
  (map (λ (l) (sort l <))
       (map (λ (s)
              (map string->number
                   (string-split s #px"\\s+" #:trim? #t)))
            (map string-trim (string-split sheet "\n")))))

;; Fold over the sorted list of lists and get the diff of the
;; high and low digits, summing them
(define (checksum sheet)
  (foldl (λ (ds result)
           (let ([low (car ds)]
                 [high (last ds)])
             (+ result (- high low))))
         0 (create-sorted-rows sheet)))
